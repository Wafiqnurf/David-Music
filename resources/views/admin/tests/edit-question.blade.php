...
<div class="card">
        ...
        <form action="{{ route('admin.tests.questions.update', [$test, $question]) }}" method="POST" id="questionForm">
            ...
            <div class="form-group mb-3">
                ...
                <div id="optionsContainer">
                    ...
                    $optionCount = max(4, count($options));
                    @endphp

                    @for($i = 0; $i < $optionCount; $i++) <div class="option-input mb-2">
                        <div class="d-flex align-items-center">
                            ...
                            <input type="text" name="options[]"
                                ...
                                placeholder="Masukkan pilihan {{ chr(65 + $i) }}" value="{{ $options[$i] ?? '' }}"
                                required>
                            @if($i >= 2)
    ...
    <div class="form-group mb-3">
        ...
        <select name="correct_answer" id="correct_answer"
            ...
            @php
            $selectedCorrectAnswer = old('correct_answer', $question->correct_answer ?? null);
            @endphp
            @foreach($options as $index => $option)
            <option value="{{ $index }}" {{ $selectedCorrectAnswer == $index ? 'selected' : '' }}>
                {{ chr(65 + $index) }}. {{ $option }}
            </option>
            @endforeach
        </select>
    ...
    <div class="form-group mb-3">
        ...
        <textarea name="explanation" id="explanation" rows="2"
            ...
            placeholder="Masukkan penjelasan jawaban (opsional)">{{ old('explanation', $question->explanation) }}</textarea>
        @error('explanation')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
const currentCorrectAnswer = {
    {
        old('correct_answer', $question - > correct_answer ?? 'null')
    }
};
...
function updateCorrectAnswerOptions() {
    ...
    } else if (currentCorrectAnswer !== null && currentCorrectAnswer !== 'null' && currentCorrectAnswer < options
        .length) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initial population of correct answer options is already done by PHP
    // Only call updateCorrectAnswerOptions when user changes input values
